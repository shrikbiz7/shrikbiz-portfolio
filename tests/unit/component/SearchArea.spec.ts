...
describe('SearchArea', () => {
    let wrapper: Wrapper<SearchArea>;
    let vuetify: any;
    describe('App should render', () => {
        ...
        it('position & location search area', () => {
            expect(wrapper.findAll('#search-area').length).to.equal(2);
        });
        it('number of buttons', () => {
            expect(wrapper.findAll('#btn').length).to.equal(1);
        });
    });
});
